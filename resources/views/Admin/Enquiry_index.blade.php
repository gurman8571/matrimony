    ...
    <div x-data="setup()" x-init="$refs.loading.classList.add('hidden'); setColors(color);" :class="{ 'dark': isDark}">
        <div class="flex h-screen antialiased text-gray-900 bg-gray-100 dark:bg-dark dark:text-light">
            ...
            <x-admin_sidebar/>
              ...
              <table class="shadow-lg table-auto">
  <thead>

    <tr>

    <th class=" bg-primary-light"width="10%">S.no</th>
      <th class=" bg-primary-light"width="10%">Username</th>
      <th class=" bg-primary-light" style="width:30%">issue</th>
      <th class=" bg-primary-light"width="10%">email</th>
      <th class=" bg-primary-light"width="10%">Phone</th>
      <th class=" bg-primary-light" width="10%">Raised on</th>

         <th class=" bg-primary-light"width="10%">Is_resolved</th>
      <th class=" bg-primary-light"width="10%">delete</th>
    </tr>
  </thead>
  <tbody>
  @foreach($data as $key=>$item)
    <tr class="text-sm">
    <td data-column="S.no" class="dark:text-dark text-sm">{{$item['id']}}</td>
      <td data-column="username text-sm" class="dark:text-dark text-sm">    {{$item['name']}}</td>
      <td data-column="issue"  class=" text-xs dark:text-dark text-xs"> {{$item['message']}} </td>

      <td data-column="email" class="  dark:text-dark text-sm"> {{$item['email']}} </td>
      <td data-column="phone" class="  dark:text-dark text-sm"> {{$item['phone']}} </td>
      <td data-column="date" class="  dark:text-dark text-sm"> {{ date("d-M-Y",strtotime($item->created_at)) }}</td>

      <td data-column="status"> @if ($item['status']==0)

          <a id="inactive"class="test-sm" href="status/{{$item->id}}" >unsolved</a>
          @else
          <a id="active" href="status/{{$item->id}}">solved</a>
      @endif
      </td>
      <td data-column="delete"  id="delete"><a href="delete/{{$item->id}}"><i class="far fa-trash-alt"style="color:red"></i></a></td>
    </tr>
    <tr>
