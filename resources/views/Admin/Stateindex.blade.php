...
<x-mobilesidebar/>
                ...
                <main>
                    ...
                    <div id="pop">
                        <h1 class="dark:text-dark"> add a new state</h1>
                            <i class="fa fa-times dark:text-dark" aria-hidden="true" id="close"></i>
                            <form method="POST" action="{{route('admin.Stateinsert')}}">
                                @csrf
    ...
    <select name="country" class="p-2 m-2 placeholder"value="{{old('country')}}" id="name" placeholder="select counrty">
...
@endforeach
    ...
    @enderror
                ...
                <div class="text-center">
                    <button id="add" type="submit" class=" mt-8"> <i class="fas fa-plus" style="color:blue"></i>add new
                        state </button>
                </div>
...
<table class="shadow-lg">
  ...
  <thead  >
    ...
    <tr>
      ...
      <th class=" bg-primary-light">Country</th>
      <th class=" bg-primary-light">status</th>
      <th class=" bg-primary-light">delete</th>
    </tr>
  </thead>
  <tbody>
  @foreach($data['state'] as $key=>$item)
    <tr>
      <td data-column="S.no" class="dark:text-dark ">{{$item['id']}}</td>
      <td data-column="name" class="dark:text-dark ">    {{$item['name']}}</td>
      <td data-column="country" class="dark:text-dark "> {{$item->country->name}}</td>
      <td data-column="status">                 <p class="{{$item['status'] == 0 ?'inactive' :'active'}} status cursor-pointer"  data-id="{{$item->id}}"  >{{$item['status'] == 0 ?'inactive' :'active'}}</p>

      </td>
      <td data-column="delete" id="delete"><a href="/admin/State/delete/{{$item['id']}}"><i class="far fa-trash-alt"style="color:red"></i></a></td>
    </tr>
    <tr>
...
<span id="links">{{$data['state']->links()}}</span>
<br>
<br>
